...
import { useNavigate } from 'react-router-dom'; // <-- FIX: Import the real hook

const UserProfile = () => {
  const navigate = useNavigate(); // <-- FIX: Use the real hook
  
...
    if (customSkill.trim() && !skills[customSkill.trim()] && !initialSkills.includes(customSkill.trim())) {
...
    // Get token (assuming you store it in localStorage after login)
    const token = localStorage.getItem('token'); 
    
    // 1. Gather all selected skills
...
      setMessage({ text: "Name and University are required fields.", type: "error" });
      return;
    }
...
      const response = await axios.put('/api/user/updateProfile', payload, config);

      if (response.status === 200 || response.status === 201) {
        setMessage({ text: response.data.message || 'Profile updated successfully!', type: 'success' });
        
        // 3. Redirect immediately to the correct user info page path: "userinfo"
        navigate('/userinfo'); 

      } else {
        setMessage({ text: response.data.error || 'Failed to update profile.', type: 'error' });
      }
    } catch (error) {
      console.error('Profile submission error:', error);
      // If the error is 401, guide the user to log in again
      const isAuthError = error.response && error.response.status === 401;
      const errorMessage = isAuthError 
        ? 'Authentication failed. Please log in again.' 
        : error.response?.data?.error || error.message || 'Network error. Please try again later.';
...
            {/* Highest Education */}
            <div>
              <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="highest_education">
                Highest Education
              </label>
              <select
